...
//   build, hoisted to the workspace root), so its `react` / `rxjs` imports
//   resolve to the copies this package uses — nothing to dedupe.
// - `@fozy-labs/statechart-converter` is the workspace link
//   `node_modules/@fozy-labs/statechart-converter` → `../converter`, resolved
//   through its `exports` to the built `dist/`: build the converter before
//   this package (the root scripts do). Vite treats a linked package as
//   source — served from `dist/` as is, never pre-bundled, a rebuilt `dist`
//   is picked up without `--force`. Its `typescript` import (a CommonJS
//   package) is found by the dependency scanner at server start, through the
//   literal `import("@fozy-labs/statechart-converter")` of
